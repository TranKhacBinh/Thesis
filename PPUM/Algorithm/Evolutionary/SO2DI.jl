module SO2DI
    ...
    function f_SO2DI(indices,
        ...
        for (nsi_idx, nsi_util_in_trans) in zip(nsis_related, nsis_util_in_trans)
            if last(nsis_util[nsi_idx]) - sum(nsi_util_in_trans[idx] for idx in indices) < min_util
                mc += 1
            end
        end
    ...
    function so2di(transactions::Vector{Transaction}, 
        ...
        for (si_idx, si) in enumerate(sis)
            ...
            if si_idx != num_sis
                for (nsi_idx, nsi_util_in_trans) in zip(nsis_related, nsis_util_in_trans)
                    new_utility = nsis_util[nsi_idx][2] - sum(nsi_util_in_trans[idx] for idx in best_indices)
